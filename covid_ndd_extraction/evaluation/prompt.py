"""Prompt comparison using BioBERT full-triple embedding + Hungarian matching."""
...
def _compare_triples_hungarian(
...
def _evaluate_prompt(
    ...
    TP, FP, FN = _compare_triples_hungarian(gold_dict, eval_dict, threshold, tokenizer, model)
    precision = TP / (TP + FP) if (TP + FP) else 0.0
    recall = TP / (TP + FN) if (TP + FN) else 0.0
    f1 = 2 * precision * recall / (precision + recall) if (precision + recall) else 0.0
    logger.info("%s — P=%.3f R=%.3f F1=%.3f (TP=%d FP=%d FN=%d)", prompt_name, precision, recall, f1, TP, FP, FN)
    return prompt_name, precision, recall, f1, TP, FP, FN


def run_prompt_assessment(
    gold_path: str,
    prompt_paths: dict[str, str],
    threshold: float = 0.85,
    output_stats: str = "data/prompt_engineering/statistical_data/Prompt_Comparison.xlsx",
    ...
    prompt_paths:
        Mapping of prompt label → path to GPT triples Excel file.
    threshold:
    ...
    for prompt_name, eval_path in prompt_paths.items():
        ...
        result = _evaluate_prompt(prompt_name, df_gold, df_eval, threshold, tokenizer, model)
    ...
    plt.savefig(os.path.join(output_figures_dir, "Prompt_Comparison.tiff"), dpi=600)
    plt.savefig(os.path.join(output_figures_dir, "Prompt_Comparison.png"), dpi=600)
    plt.close()

    os.makedirs(os.path.dirname(output_stats), exist_ok=True)
    df_stats.to_excel(output_stats, index=False)
    logger.info("Prompt assessment complete.")
